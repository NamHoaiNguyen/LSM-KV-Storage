...
std::unique_ptr<TableReader> CreateAndSetupDataForTableReader(std::string&& filename, SSTId table_id, uint64_t file_size) {
  ...
  auto new_table_reader = std::make_unique<TableReader>(std::move(table_reader_data));
...
void DecodeExtraInfo(TableReaderData* table_reader_data) {
  ...
  ssize_t bytes_read =
      table_reader_data->read_file_object->RandomRead(extra_info_buffer, start_offset_extra_info);
  if (bytes_read < 0) {
    return;
  }

  // first 8 bytes contains info of total block entries in table
  uint64_t total_block_entries = *reinterpret_cast<uint64_t *>(&extra_info_buffer[0]);
  ...
  uint64_t meta_section_length = *reinterpret_cast<uint64_t *>(&extra_info_buffer[16]);
  // byte 24- 31 contains min transaction id
  table_reader_data->min_transaction_id = *reinterpret_cast<uint64_t *>(&extra_info_buffer[24]);
  // byte 32 - 40 contain max transcation id
  table_reader_data->max_transaction_id = *reinterpret_cast<uint64_t *>(&extra_info_buffer[32]);
...
void FetchBlockIndexInfo(uint64_t total_block_entries,
  ...
  for (int i = 0; i < total_block_entries; i++) {
    ...
    table_reader_data->block_index_.emplace_back(block_smallest_key, block_largest_key,
                                                 block_starting_offset, block_length);
  }
}

TableReader::TableReader(std::unique_ptr<TableReaderData> table_reader_data)
    : filename_(std::move(table_reader_cache_->filename)),
      table_id_(table_reader_data->table_id),
      file_size_(table_reader_data->file_size),
      min_transaction_id_(table_reader_data->min_transaction_id_),
      max_transaction_id_(table_reader_data->max_transaction_id_),
      block_index_(std::move(table_reader_data->block_index_)),
      read_file_object_(std::move(table_reader_cache_->read_file_object_)) {}
