...
void BlockIndex::AddEntry(std::string_view first_key, std::string_view last_key,
  ...
  const Byte *const first_key_buff = reinterpret_cast<const Byte *const>(first_key.data());
  ...
  // Insert first key
  buffer_.insert(buffer_.end(), first_key_buff,
                 first_key_buff + first_key_len_bytes);
   // Insert length of last key(4 Bytes)
  buffer_.insert(buffer_.end(), last_key_len_bytes,
                 last_key_len_bytes + sizeof(uint32_t));
  // Insert last key
  buffer_.insert(buffer_.end(), last_key_buff,
                 last_key_buff + last_key_len_bytes);
...
std::span<const Byte> BlockIndex::GetBufferView() {
  return buffer_;
}
