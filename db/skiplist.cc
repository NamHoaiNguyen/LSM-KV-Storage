...
SkipList::SkipList()
    : current_level_(1), max_level_(MAX_LEVEL),
      ...
      current_size_(0),
      head_(std::make_shared<SkipListNode>("" /*key*/, "" /*value*/, current_level_)) {}
...
std::optional<std::string> SkipList::Get(std::string_view key) {
  ...
  for (int level = max_level_ - 1; level >= 0; --level) {
    while (current->forward_[level] && current->forward_[level] < key) {
...
bool SkipList::Put(std::string_view key, std::string_view value) {
  ...
  std::vector<std::shared_ptr<SkipListNode>> updates(max_level, std::nullptr);
  auto new_node = std::make_shared<SkipListNode>(key, value, new_level);
  
  std::shared_ptr<SkipListNode> current = head_;
  for (int level = max_level_ - 1; level >= 0; --level) {
    while (current->forward_[level] && current->forward_[level]->key < key) {
      current = current->forward_[level];
    }
  ...
  if (current->forward_[0] && current->forward_[0]->key == key) {
...
std::vector<std::shared_ptr<SkipListNode>>
  ...
  std::vector<std::shared_ptr<SkipListNode>> prev_nodes(max_level, std::nullptr);
