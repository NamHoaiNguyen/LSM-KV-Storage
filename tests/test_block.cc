...
TEST(BlockTest, BlockReaderIterator) {
  ...
  for (int i = 0; i < block_index.size(); i++) {
    ...
    // correponding blockreaderiterator
    ...
    auto iterator = std::make_unique<sstable::BlockReaderIterator>(
        std::move(lru_block_item));
    ...
    for (iterator->SeekToFirst(); iterator->IsValid(); iterator->Next()) {
      ...
      EXPECT_EQ(value_found, db->Get(key_found, 0 /*txn_id*/).value.value());
